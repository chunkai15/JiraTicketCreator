    ...
    content: [
      {
        type: "heading",
        attrs: { level: 1 },
        content: [{ type: "text", text: `Release checklist for the ${releaseName}` }]
      },
      {
        type: "paragraph",
      ...
      {
        type: "paragraph",
        content: [{ type: "text", text: `Generated by Release Page Creator Tool - ${new Date().toISOString()}`, marks: [{ type: "em" }] }]
      }
    ]
  };
}
...
app.post('/api/confluence/create-page', async (req, res) => {
  try {
    ...
    // Add timestamp to ensure title uniqueness with millisecond precision
    const now = new Date();
    const timestamp = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')} ${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}:${String(now.getSeconds()).padStart(2, '0')}.${String(now.getMilliseconds()).padStart(3, '0')}`;
    const uniqueTitle = `${title} - ${timestamp}`;
    
    console.log(`Creating Confluence page: ${uniqueTitle} in space: ${spaceKey}`);
    ...
    const pageUrl = `${baseURL}/wiki/spaces/${spaceKey}/pages/${createdPage.id}/${encodeURIComponent(uniqueTitle.replace(/\s+/g, '+'))}`;
